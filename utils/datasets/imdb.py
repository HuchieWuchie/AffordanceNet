...
class imdb(object):
    ...
    def evaluate_detections(self, all_boxes, all_rles = None, output_dir=None):
        """
    ...
    def append_flipped_images(self):
        ...
        for i in range(num_images):
            ...
            oldx1 = boxes[:, 0].copy()
            oldx2 = boxes[:, 2].copy()
            boxes[:, 0] = widths[i] - oldx2 - 1 #vertical flip (flip theo width, tuc lat anh trai-->phai ---> thay doi x, chu khong thay doi y)
            ...
            if not cfg.MASK_REG:
                entry = {'boxes' : boxes,
                         ...
                         'flipped' : True #--->flag day la anh do flipped
                         }
            else:
                seg_mask_inds = self.roidb[i]['seg_mask_inds'].copy()
                entry = {'boxes': boxes,
                         'gt_overlaps': self.roidb[i]['gt_overlaps'],
                         'gt_classes': self.roidb[i]['gt_classes'],
                         'flipped': True, #--->flag day la anh do flipped
    ...
    def merge_roidbs(a, b):
        ...
        for i in range(len(a)):
            ...
            a[i]['seg_areas'] = np.hstack((a[i]['seg_areas'],
                                          b[i]['seg_areas']))
            if cfg.MASK_REG:
                a[i]['seg_mask_inds'] = np.hstack((a[i]['seg_mask_inds'],
                                               b[i]['seg_mask_inds']))
        return a
    ...
    def load_image(self, image_id):
        ...
        """
        # Load image
        image = skimage.io.imread(self.image_path_at(image_id))
